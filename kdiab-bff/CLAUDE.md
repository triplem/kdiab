...
**kdiab-bff** is a stateless Backend-for-Frontend (BFF) that aggregates data from all three kdiab services and provides a unified analytics dashboard. It has **no database** — all state comes from upstream HTTP calls.
...
The BFF forwards the user's `Authorization: Bearer <token>` header unchanged to all upstream services. For this to work, the JWT must carry all four audiences: `bff`, `measure`, `profile`, `treatment`.

- **Root compose**: the `kdiab-bff-frontend` Keycloak client in `config/keycloak-realm.json` has four audience mappers — tokens issued at login are accepted by all four backends automatically.
- **Standalone compose**: the `kdiab-bff-frontend` client in `kdiab-bff/config/keycloak-realm.json` only has the `bff` audience. Upstream services must be configured to trust the same realm, or use a shared Keycloak instance.

## Backend Package Structure

Root package: `org.javafreedom.kdiab.bff`
...
- **No database**: the BFF is purely a read-time aggregator. Stateless — safe to scale horizontally.
- **No generated Paths**: `api/openapi.yaml` is used only for TypeScript client generation and Swagger UI. The backend uses manual Ktor routing (not the OpenAPI server stubs) because the BFF aggregation logic doesn't map cleanly to generated controller stubs.
