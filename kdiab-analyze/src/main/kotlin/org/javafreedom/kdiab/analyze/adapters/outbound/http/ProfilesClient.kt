...
private fun Profile.toDomain() = UpstreamProfile(
    ...
    durationOfAction = settings.durationOfAction,
    analysisLow = analysisRange?.low ?: analysisLow,
    analysisHigh = analysisRange?.high ?: analysisHigh,
    createdAt = createdAt,
    validFrom = validFrom,
    previousProfileId = previousProfileId,
    activatedAt = activatedAt,
    archivedAt = archivedAt,
    basal = schedule.basal.map { BasalSegment(it.startTime, it.value) },
    icr = schedule.icr.map { RatioSegment(it.startTime, it.value) },
    isf = schedule.isf.map { RatioSegment(it.startTime, it.value) },
    targets = schedule.targets.map { TargetSegment(it.startTime, it.low, it.high) },
)
