# Epics Index

| Epic | Title | Status | Milestone | Stories |
|---|---|---|---|---|
| [#288](https://github.com/triplem/kdiab/issues/288) | Adopt Nightscout API v3 in kdiab-nightscout | Closed | M1 | Multiple — see epic |
| [#968](https://github.com/triplem/kdiab/issues/968) | UI bug fixes from manual testing round — time format, basal chart, CGM dashboard | Closed | M2 | #969, #970, #971, #972, #973, #974, #975 |
| [#1005](https://github.com/triplem/kdiab/issues/1005) | E2E & Regression Test Coverage for kdiab-ui | Closed | M3 | #1006, #1007, #1008, #1009, #1010, #1011, #1012, #1013, #1014, #1015, #1028 |
| [#1016](https://github.com/triplem/kdiab/issues/1016) | UI Bug Fixes — Manual Testing Round 3 | Open (in-progress) | M4 | #1017, #1018, #1019, #1020, #1021 |
| [#1038](https://github.com/triplem/kdiab/issues/1038) | UX & Platform Enhancements | Closed | M5 | #17, #19, #24, #30 |
| [#1365](https://github.com/triplem/kdiab/issues/1365) | Gradle Build Infrastructure — build-logic convention plugins | Open (pending-approval) | M6 | TBD |
| [#1366](https://github.com/triplem/kdiab/issues/1366) | API Client Distribution — generated Kotlin client JARs per service | Open (pending-approval) | M6 | TBD |
| [#1367](https://github.com/triplem/kdiab/issues/1367) | Consumer Service Migration to Generated Clients + Server Codegen Completion | Open (pending-approval) | M6 | TBD |

## Legend

- **Open (backlog)** — epic and stories created, not yet started
- **Open (pending-approval)** — epic created, stories written, awaiting `approved` label from human
- **Open (in-progress)** — at least one story branch is active or a PR is open
- **Closed** — all stories merged, epic closed
