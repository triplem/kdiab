...
- **Model access**: This project runs on Anthropic's default models through the standard Claude Code provider — no AWS Bedrock and no Amazon-hosted models. `.claude/settings.json` defaults the orchestrator to Opus 4.8 with the 1M-context variant (`opus[1m]`). You need standard Claude Code access (an Anthropic API key or a Claude subscription); no AWS account, credentials, or region setup is required. Override the model per-developer in `.claude/settings.local.json`.
...
- **Agents**: `.claude/agents/` — 10 domain-expert personas (product, design, delivery, architect, compliance, devsecops, developer, quality, pipeline-deploy, operations). Each is a flat `.md` file prefixed `aidlc-<role>-agent.md`; the conductor adopts the persona inline, or delegates to it via the `Task` tool for the two subagent stages (2.1, 3.5). (The `aidlc-aws-platform-agent` was removed — this project has no AWS infrastructure; see `project.md` § Forbidden.)
